"""
V18 Titan Model Export Script

Uses pre-computed OOF/test checkpoints from v18_titan_solution.py to:
...
Architecture:
- 36 models (HGB × 3 seeds, LGB × 12, XGB × 6, CatBoost × 15)
- 20-fold cross-validation with seed averaging
- HuberRegressor(ε=1.35) meta-model

Checkpoint directory: titan_checkpoints/
Output: mlops/artifacts/v18/ directory
"""
...
try:
    ...
    HAS_XGB = False
    print("WARNING: xgboost not installed. XGB models will be skipped in full-data retraining.")
...
OUTPUT_DIR = os.path.join("mlops", "artifacts", "v18")
...
if not oof_files:
    ...
    print("Run `python v18_titan_solution.py` first, or copy checkpoints here.")
    exit(1)
...
# ─── Step 2: Feature Engineering (same as V13/V18) ─────────────
...
gm = y.mean()
print(f"Computing {N_FOLDS}-fold target encodings + interactions...")
te_store = {}
te_stats_all = {}
...
print(f"[TITAN STACK] OOF RMSE = {stack_rmse:.5f} | R² = {stack_r2:.4f}")
...
for m_name in model_names:
    ...
    family = parts[0]  # hgb, lgb, cat, xgb
    start = time.time()
    ...
    try:
        if family == 'hgb':
            ...
            variant = parts[1]  # mae, rmse, deep, dart
            ...
            if variant == 'mae':
                ...
                print(f"  [SKIP] Unknown LGB variant: {m_name}")
                continue
            ...
            elif variant in ('huber', 'hub'):
                p = {'objective': 'reg:pseudohubererror', 'eval_metric': 'rmse',
                     'learning_rate': 0.03, 'max_depth': 6, 'subsample': 0.8,
                     'colsample_bytree': 0.7, 'alpha': 0.1, 'lambda': 1.0,
                     'seed': seed, 'tree_method': 'hist'}
            else:
                print(f"  [SKIP] Unknown XGB variant: {m_name}")
                continue
            ...
            # Parse depth: cat_d{depth}_s{seed}
            ...
            print(f"  [SKIP] {m_name} (unsupported family: {family})")
            continue

        elapsed = time.time() - start
        print(f"  [OK] {m_name} ({elapsed:.1f}s)")
...
class InferencePipeline:
    """V18 Titan inference pipeline: base models + HuberRegressor meta."""
...
# Only include models that were successfully retrained
...
print(f"\n{'=' * 60}")
print(f"V18 TITAN MODEL EXPORT COMPLETE")
print(f"{'=' * 60}")
